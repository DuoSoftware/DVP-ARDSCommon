...
var AddRequestToQueue = function (logKey, request, callback) {
  logger.info(
    "%s ************************* Start AddRequestToQueue *************************",
    logKey,
  );

  redisHandler.AddItemToListR(
    logKey,
    request.QueueId,
    request.SessionId,
    function (err, result) {
      if (err) {
        ...
        if (parseInt(result) > 0) {
          ...
          requestHandler.SetRequestState(
            logKey,
            request.Company,
            request.Tenant,
            request.SessionId,
          ...
          var hashKey = util.format(
            "ProcessingHash:%d:%d:%s",
            request.Company,
            request.Tenant,
            request.RequestType,
          );
          SetNextProcessingItem(
            logKey,
            request.QueueId,
            hashKey,
            "CreateHash",
            function (err, result) {
              ...
              if (request.QPositionEnable) {
                SendQueuePositionInfo(
                  logKey,
                  request.QPositionUrl,
                  request.QueueId,
                  request.CallbackOption,
                  function () {},
                );
              }
...
var RemoveRequestFromQueue = function (
  logKey,
  company,
  tenant,
  businessUnit,
  ...
  logger.info(
    "%s ************************* Start RemoveRequestFromQueue *************************",
    logKey,
  );
  ...
  redisHandler.RemoveItemFromList(
    logKey,
    queueId,
    sessionId,
    function (err, result) {
      if (err) {
        ...
        if (result > 0) {
          ...
          var hashKey = util.format(
            "ProcessingHash:%s:%s:%s",
            company,
            tenant,
            requestType,
          );
          var queuePositionUrl = util.format(
            "%s:%s/api/ards_position",
            config.Services.callServerUrl,
            config.Services.callServerPort,
          );
          SetNextProcessingItem(
            logKey,
            queueId,
...
var GetNextRequestToProcess = function (logKey, queueId, callback) {
  logger.info(
    "%s ************************* Start GetNextRequestToProcess *************************",
    logKey,
  );
  var rejectedQueueId = GetRejectedQueueId(queueId);

  redisHandler.GetItemFromList(
    logKey,
    rejectedQueueId,
    function (err, rejectListResult) {
      if (err) {
...
var SetNextProcessingItem = function (
  logKey,
  queueId,
  processingHashId,
  currentSession,
  ...
  redisHandler.RLock.acquire([redLokKey], 500).then(function (lock) {
    ...
    redisHandler.GetObj(logKey, queueExecuteCount, function (err, exeCountStr) {
      ...
      redisHandler.HScanHash(
        logKey,
        processingHashId,
        hscanPattern,
        function (processingHashDetail) {
          if (processingHashDetail) {
            var GetNextItemToProcess = function (callback) {
              ...
              redisHandler.GetItemFromList(
                logKey,
                rejectedQueueId,
                function (err, nextRejectQueueItem) {
                  if (err) {
                    ...
                    if (
                      nextRejectQueueItem == "" ||
                      nextRejectQueueItem == null
                    ) {
                      redisHandler.GetItemFromList(
                        logKey,
                        queueId,
                        function (err, nextQueueItem) {
                          if (err) {
                            logger.error(
                              "GetNextItemToProcess failed:: " + err,
                            );
                            callback(null);
                          } else {
            ...
            var SetNextItem = function (callback) {
              if (
                processingHashDetail.MatchingValues &&
                processingHashDetail.MatchingValues.indexOf(currentSession) > -1
              ) {
                var hashRecords = processingHashDetail.MatchingKeyValues.filter(
                  function (item) {
                ...
                if (hashRecords && hashRecords.length > 0) {
                  ...
                  GetNextItemToProcess(function (nextItem) {
                    if (nextItem) {
                      ...
                      redisHandler.RemoveItemFromHash(
                        logKey,
                        processingHashId,
                        fieldKey,
                        function (err, result) {
            ...
            if (exeCount === processingHashDetail.ItemCount) {
              logger.info(
                "exeCount === processingHashDetail.ItemCount :: Start SetNextItem",
              );
              ...
              SetNextItem(function (setNextStatus) {
                ...
                if (asyncFuncList.length > 0) {
                  ...
                  logger.info(
                    "Add new item process finished :: No new items found",
                  );
                  lock.release().catch(function (err) {
                    logger.error("Release redis lock error:: " + err);
                  });
              ...
              logger.info(
                "exeCount < processingHashDetail.ItemCount :: Start remove slot from processing hash",
              );
              if (
                processingHashDetail.MatchingValues &&
                processingHashDetail.MatchingValues.indexOf(currentSession) > -1
                ...
                if (hashRecords && hashRecords.length > 0) {
                  ...
                  redisHandler.RemoveItemFromHash(
                    ...
                    function (err, result) {
                      if (err) {
                        logger.error(
                          "SetNextItem process finished :: RemoveItemFromHash failed:: " +
                            err,
                        );
                        lock.release().catch(function (err) {
                          logger.error("Release redis lock error:: " + err);
                        ...
                        if (result === 1) {
                          logger.info(
                            "SetNextItem process finished :: Remove HashField Success.." +
                              processingHashId +
                              "::" +
                              fieldKey,
                          );
                          ...
                          logger.info(
                            "SetNextItem process finished :: Remove HashField Failed.." +
                              processingHashId +
                              "::" +
                              fieldKey,
                          );
                  ...
                  logger.info(
                    "SetNextItem process finished :: No session found ,ignore RemoveItem",
                  );
                  callback(null, "done");
                }
              } else {
                ...
                logger.info(
                  "SetNextItem process finished :: Session Mismatched ,ignore RemoveItem",
                );
                callback(null, "done");
              }
            }
            ...
            logger.info(
              "No processing hash detail found, ignore set next item",
            );
            lock.release().catch(function (err) {
              logger.error("Release redis lock error:: " + err);
            });
...
var SendQueuePositionInfo = function (
  logKey,
  url,
  queueId,
  callbackOption,
  ...
  logger.info(
    "%s:Queue: %s ************************* Start SendQueuePositionInfo *************************",
    logKey,
    queueId,
  );
        ...
        if (err) {
          ...
          if (result) {
            ...
            if (callbackOption == "GET") {
              ...
              restClientHandler.DoPostDirect(
                url,
                RequestPositionList,
                function (err, res, result) {
                  if (err) {
...
var GetProcessingQueueInfo = function (logKey, queueId, callback) {
  logger.info(
    "%s:Queue: %s ************************* Start SendProcessingQueueInfo *************************",
    logKey,
    queueId,
  );
