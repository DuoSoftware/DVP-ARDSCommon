...
var SetTags = function (logKey, tagKey, objKey, callback) {
  ...
  logger.info(
    "%s SetTags - objkey: %s :: tagkey: %s :: tagMetakey: %s",
    logKey,
    objKey,
    tagKey,
    tagMeta,
  ...
  client.get(tagMeta, function (err, result) {
    if (err) {
      ...
      logger.info(
        "%s SetTags Get tag meta - Result: %s",
        logKey,
        result || "Not found.",
      );
      if (result == null) {
        client.set(tagKey, objKey, function (err, reply) {
          if (err) {
            ...
            logger.info(
              "%s SetTags Set new tag success. - Result: %s",
              logKey,
              reply,
            );
            ...
            client.set(tagMeta, tagKey, function (err, reply) {
              if (err) {
                logger.error(
                  "%s SetTags Set tag meta - Error: %s",
                  logKey,
                  err,
                );
...
var SetObj = function (logKey, key, obj, callback) {
  redlock.acquire(["lock:" + key], 500).then(function (lock) {
    logger.info(
      "%s --------------------------------------------------",
      logKey,
    );
      ...
      SetTags(logKey, tagkey, key, function (err, reply) {
        if (err) {
          ...
          client.set(key, obj, function (error, reply) {
            ...
            if (error) {
              logger.error(
                "%s AddObj_T Error - key: %s :: Error: %s",
                logKey,
                key,
                error,
              );
      ...
      SetTags(logKey, tagkey, key, function (err, reply) {
        if (err) {
          ...
          client.set(key, obj, function (error, reply) {
            ...
            if (error) {
              logger.error(
                "%s SetObj_T Error - key: %s :: Error: %s",
                logKey,
                key,
                error,
              );
  ...
  redlock.acquire(["lock:" + key], 500).then(function (lock) {
    ...
    if (Array.isArray(tags)) {
      ...
      client.get(tagMeta, function (err, result) {
        if (err) {
          logger.error(
            "%s RemoveObj_T Remove tag Error - key: %s :: Error: %s",
            logKey,
            key,
            err,
          );
          ...
          client.del(tagMeta, function (err, reply) {});
          logger.info(
            "%s RemoveObj_T Remove tag success - key: %s :: tagkey: %s",
            logKey,
            key,
            result,
          );
    ...
    client.del(key, function (err, reply) {
      ...
      if (err) {
        logger.error(
          "%s RemoveObj_T Error - key: %s :: Error: %s",
          logKey,
          key,
          err,
        );
...
var AddObj_V = function (logKey, key, obj, callback) {
  ...
  redlock.acquire(["lock:" + key], 500).then(function (lock) {
    ...
    client.set(versionkey, vid, function (err, reply) {
      if (err) {
        logger.error(
          "%s AddObj_V SetVersion Error - key: %s :: Error: %s",
          logKey,
          key,
          err,
        );
        ...
        client.set(key, obj, function (error, reply) {
          ...
          if (error) {
            ...
            logger.error(
              "%s AddObj_V Error - key: %s :: Error: %s",
              logKey,
              key,
              error,
            );
...
var SetObj_V = function (logKey, key, obj, cvid, callback) {
  ...
  redlock.acquire(["lock:" + key], 500).then(function (lock) {
    ...
    client.get(versionkey, function (err, reply) {
      if (err) {
        logger.error(
          "%s SetObj_V GetVersion Error - key: %s :: Error: %s",
          logKey,
          key,
          err,
        );
        ...
        client.incr(versionkey, function (err, reply) {
          if (err) {
            logger.error(
              "%s SetObj_V SetVersion Error - key: %s :: Error: %s",
              logKey,
              key,
              err,
            );
            ...
            client.set(key, obj, function (error, reply) {
              ...
              if (error) {
                logger.error(
                  "%s SetObj_V Error - key: %s :: Error: %s",
                  logKey,
                  key,
                  error,
                );
        ...
        logger.info(
          "%s SetObj_V VERSION_MISMATCHED - key: %s :: cvid: %s",
          logKey,
          key,
          cvid,
        );
      ...
      SetTags(logKey, tagkey, key, function (err, reply) {
        if (err) {
          ...
          client.set(versionkey, vid, function (err, reply) {
            if (err) {
              logger.error(
                "%s AddObj_V_T SetVersion Error - key: %s :: Error: %s",
                logKey,
                key,
                err,
              );
              ...
              client.set(key, obj, function (error, reply) {
                ...
                if (error) {
                  logger.error(
                    "%s AddObj_V_T Error - key: %s :: Error: %s",
                    logKey,
                    key,
                    error,
                  );
...
var SetObj_V_T = function (logKey, key, obj, tags, cvid, callback) {
  ...
  redlock.acquire(["lock:" + key], 500).then(function (lock) {
    ...
    logger.info(
      "%s SetObj_V_T - key: %s :: obj: %s :: cvid: %s",
      logKey,
      key,
      obj,
      cvid,
    ...
    client.get(versionkey, function (err, reply) {
      if (err) {
        logger.error(
          "%s SetObj_V_T GetVersion Error - key: %s :: Error: %s",
          logKey,
          key,
          err,
        );
        ...
        dlock.release().catch(function (err) {
          logger.error("Release lock failed:: " + err);
        });
          ...
          SetTags(logKey, tagkey, key, function (err, reply) {
            if (err) {
              ...
              client.incr(versionkey, function (err, reply) {
                if (err) {
                  logger.error(
                    "%s SetObj_V_T SetVersion Error - key: %s :: Error: %s",
                    logKey,
                    key,
                    err,
                  );
                  ...
                  client.set(key, obj, function (error, reply) {
                    ...
                    if (error) {
                      logger.error(
                        "%s SetObj_V_T Error - key: %s :: Error: %s",
                        logKey,
                        key,
                        error,
                      );
...
var CheckObjExists = function (logKey, key, callback) {
  ...
  client.exists(key, function (error, reply) {
    if (error) {
      logger.error(
        "%s CheckObjExists Error - key: %s :: Error: %s",
        logKey,
        key,
        error,
      );
      ...
      logger.info(
        "%s CheckObjExists - key: %s :: Reply: %s",
        logKey,
        key,
        reply,
      );
  ...
  redlock.acquire(["lock:" + key], 500).then(function (lock) {
    client.rpush(key, obj, function (error, reply) {
      if (error) {
        ...
        logger.error(
          "%s AddItemToListR Error - key: %s :: Error: %s",
          logKey,
          key,
          error,
        );
        ...
        logger.info(
          "%s AddItemToListR - key: %s :: Reply: %s",
          logKey,
          key,
          reply,
        );
  ...
  redlock.acquire(["lock:" + key], 500).then(function (lock) {
    client.lpush(key, obj, function (error, reply) {
      if (error) {
        ...
        logger.error(
          "%s AddItemToListL Error - key: %s :: Error: %s",
          logKey,
          key,
          error,
        );
        ...
        logger.info(
          "%s AddItemToListL - key: %s :: Reply: %s",
          logKey,
          key,
          reply,
        );
...
var GetItemFromList = function (logKey, key, callback) {
  ...
  client.lpop(key, function (err, result) {
    if (err) {
      logger.error(
        "%s GetItemFromList Error - key: %s :: Error: %s",
        logKey,
        key,
        err,
      );
      ...
      logger.info(
        "%s GetItemFromList - key: %s :: Reply: %s",
        logKey,
        key,
        result,
      );
...
var GetLengthOfTheList = function (logKey, key, callback) {
  ...
  client.llen(key, function (err, result) {
    if (err) {
      logger.error(
        "%s GetLengthOfTheList Error - key: %s :: Error: %s",
        logKey,
        key,
        err,
      );
      ...
      logger.info(
        "%s GetLengthOfTheList - key: %s :: Reply: %s",
        logKey,
        key,
        result,
      );
...
var GetRangeFromList = function (logKey, key, callback) {
  ...
  client.lrange(key, 0, -1, function (err, result) {
    if (err) {
      logger.error(
        "%s GetRangeFromList Error - key: %s :: Error: %s",
        logKey,
        key,
        err,
      );
      ...
      logger.info(
        "%s GetRangeFromList - key: %s :: Reply: %s",
        logKey,
        key,
        result,
      );
  ...
  redlock.acquire(["lock:" + key], 500).then(function (lock) {
    client.lrem(key, 0, obj, function (err, result) {
      if (err) {
        ...
        logger.error(
          "%s RemoveItemFromList Error - key: %s :: Error: %s",
          logKey,
          key,
          err,
        );
        ...
        logger.info(
          "%s RemoveItemFromList - key: %s :: Reply: %d",
          logKey,
          key,
          result,
        );
...
var AddItemToHash = function (logKey, hashKey, field, obj, callback) {
  logger.info("%s --------------------------------------------------", logKey);
  logger.info(
    "%s AddItemToHash - hashKey: %s :: field: %s :: obj: %s",
    logKey,
    hashKey,
    field,
    obj,
  ...
  redlock.acquire(["lock:" + hashKey], 500).then(function (lock) {
    client.hset(hashKey, field, obj, function (err, result) {
      if (err) {
        logger.error(
          "%s AddItemToHash Error - hashKey: %s :: field: %s  :: Error: %s",
          logKey,
          hashKey,
          field,
          err,
        ...
        logger.info(
          "%s AddItemToHash - hashKey: %s :: field: %s  :: Reply: %s",
          logKey,
          hashKey,
          field,
          result,
...
var RemoveItemFromHash = function (logKey, hashKey, field, callback) {
  logger.info("%s --------------------------------------------------", logKey);
  logger.info(
    "%s RemoveItemFromHash - hashKey: %s :: field: %s",
    logKey,
    hashKey,
    field,
  );
  ...
  redlock.acquire(["lock:" + hashKey], 500).then(function (lock) {
    client.hdel(hashKey, field, function (err, result) {
      if (err) {
        logger.error(
          "%s RemoveItemFromHash Error - hashKey: %s :: field: %s  :: Error: %s",
          logKey,
          hashKey,
          field,
          err,
        ...
        logger.info(
          "%s RemoveItemFromHash - hashKey: %s :: field: %s  :: Reply: %s",
          logKey,
          hashKey,
          field,
          result,
...
var RemoveHash = function (logKey, hashKey, callback) {
  ...
  redlock.acquire(["lock:" + hashKey], 500).then(function (lock) {
    client.del(hashKey, function (err, result) {
      if (err) {
        logger.error(
          "%s RemoveHash Error - hashKey: %s :: Error: %s",
          logKey,
          hashKey,
          err,
        );
        ...
        logger.info(
          "%s RemoveHash - hashKey: %s :: Reply: %s",
          logKey,
          hashKey,
          result,
        );
...
var CheckHashFieldExists = function (logKey, hashkey, field, callback) {
  logger.info("%s --------------------------------------------------", logKey);
  logger.info(
    "%s CheckHashFieldExists - hashKey: %s :: field: %s",
    logKey,
    hashkey,
    field,
  );
  CheckObjExists(logKey, hashkey, function (err, hresult) {
    if (err) {
      logger.error(
        "%s CheckHashExists Error - hashKey: %s:: Error: %s",
        logKey,
        hashkey,
        err,
      );
      ...
      logger.info(
        "%s CheckHashExists - hashKey: %s:: Reply: %s",
        logKey,
        hashkey,
        hresult,
      );
      ...
      client.hexists(hashkey, field, function (err, result) {
        if (err) {
          logger.error(
            "%s CheckHashFieldExists Error - hashKey: %s :: field: %s  :: Error: %s",
            logKey,
            hashkey,
            field,
            err,
          ...
          logger.info(
            "%s CheckHashFieldExists - hashKey: %s :: field: %s  :: Reply: %s",
            logKey,
            hashkey,
            field,
            result,
...
var GetHashValue = function (logKey, hashKey, field, callback) {
  logger.info("%s --------------------------------------------------", logKey);
  logger.info(
    "%s GetHashValue - hashKey: %s :: field: %s",
    logKey,
    hashKey,
    field,
  );
  ...
  client.hget(hashKey, field, function (err, result) {
    if (err) {
      logger.error(
        "%s GetHashValue Error - hashKey: %s :: field: %s  :: Error: %s",
        logKey,
        hashKey,
        field,
        err,
      ...
      logger.info(
        "%s GetHashValue - hashKey: %s :: field: %s  :: Reply: %s",
        logKey,
        hashKey,
        field,
        result,
...
var GetAllHashValue = function (logKey, hashkey, callback) {
  ...
  client.hvals(hashKey, function (err, result) {
    if (err) {
      logger.error(
        "%s GetAllHashValue Error - hashKey: %s :: Error: %s",
        logKey,
        hashKey,
        err,
      );
      ...
      logger.info(
        "%s GetAllHashValue - hashKey: %s :: Reply: %s",
        logKey,
        hashKey,
        result,
      );
...
var HScanHash = function (logKey, hashKey, pattern, callback) {
  logger.info("%s --------------------------------------------------", logKey);
  logger.info(
    "%s HScanHash - hashKey: %s :: pattern: %s",
    logKey,
    hashKey,
    pattern,
  );
  ...
  stream.on("data", function (resultKeys) {
    for (var i = 0; i < resultKeys.length; i += 2) {
      response.MatchingKeyValues.push({
        Field: resultKeys[i],
        Value: resultKeys[i + 1],
      });
...
var Publish = function (logKey, pattern, message, callback) {
  logger.info("%s --------------------------------------------------", logKey);
  logger.info(
    "%s Redis Publish - pattern: %s - message: %s",
    logKey,
    pattern,
    message,
  );
  ...
  client.publish(pattern, message, function (err, result) {
    if (err) {
      logger.error(
        "%s Redis Publish Error - pattern: %s :: Error: %s",
        logKey,
        pattern,
        err,
      );
      ...
      logger.info(
        "%s Redis Publish - pattern: %s :: Reply: %s",
        logKey,
        pattern,
        result,
      );
