        ...
        switch (eventType) {
          case "Connected":
            if (productiveItems.length === 1) {
              ...
              dashboardEventHandler.PublishEvent(
                "DashBoardEvent",
                tenantInt,
                companyInt,
                businessUnit,
  ...
  process.nextTick(function () {
    if (Array.isArray(taskInfos) && taskInfos.length > 0) {
      ...
      for (var i in taskInfos) {
        ...
        var attributes = [];
        var validateHandlingType = commonMethods.FilterByID(
          loginTask,
          "Type",
          taskInfo.ResTask.ResTaskInfo.TaskType
        );
        if (validateHandlingType) {
          resourceService.GetResourceAttributeDetails(
            accessToken,
            taskInfo,
            function (resAttErr, resAttRes, resAttObj, reTaskInfo) {
              var task = {
              ...
              if (resAttErr) {
                ...
                var ppad = PreProcessAttributeData(
                  task.HandlingType,
                  resAttObj.Result.ResResourceAttributeTask
                );
  ...
  process.nextTick(function () {
    if (Array.isArray(attributeInfos) && attributeInfos.length > 0) {
      ...
      for (var i in attributeInfos) {
        ...
        if (
          attributeInfo &&
          attributeInfo.Percentage &&
          attributeInfo.Percentage > 0
        ) {
          var attribute = {
...
var PreProcessResourceData = function (
  logKey,
  accessToken,
  preResourceData,
  loginTask,
  ...
  resourceService.GetResourceTaskDetails(
    accessToken,
    preResourceData.ResourceId,
    function (taskErr, taskRes, taskObj) {
      ...
      if (taskErr) {
        ...
        if (taskObj.IsSuccess) {
          ...
          pptd.on("endTaskInfo", function () {
            ...
            preResourceData.ResourceAttributeInfo.forEach(function (
              attributeObj
            ) {
        ...
        if (err) {
          ...
          redisHandler.GetObj(logKey, tagMetaKey, function (err, reTags) {
            if (err) {
              ...
              if (reTags) {
                commonMethods.ConvertTagStrToArray(reTags, function (
                  slotInfoTags
                ) {
...
var RemoveResourceState = function (
  logKey,
  company,
  tenant,
  resourceid,
...
var SetResourceLogin = function (logKey, basicData, callback) {
  ...
  var preResourceData = {};
  resourceService.GetResourceDetails(
    accessToken,
    basicData.ResourceId,
    function (resErr, resRes, resObj) {
      if (resErr) {
        ...
        if (resObj.IsSuccess) {
          ...
          preResourceData = {
            ...
            BusinessUnit: basicData.BusinessUnit
              ? basicData.BusinessUnit
              : "default",
          ...
          PreProcessResourceData(
            ...
            accessToken,
            preResourceData,
            basicData.HandlingTypes,
            function (err, msg, preProcessResData) {
              if (err) {
                  ...
                  if (validateHandlingType) {
                    ...
                    for (var i = 0; i < obj.NoOfSlots; i++) {
                      var slotInfokey = util.format(
                        "CSlotInfo:%d:%d:%s:%s:%d",
                        preProcessResData.Company,
                        preProcessResData.Tenant,
                        preProcessResData.ResourceId,
                        obj.HandlingType,
                        i
                      );
                      var slotInfo = {
                      ...
                      var slotInfoTags = [
                        "company_" + slotInfo.Company,
                        "tenant_" + slotInfo.Tenant,
                        "handlingType_" + slotInfo.HandlingType,
                        "state_" + slotInfo.State,
                        "resourceid_" + preProcessResData.ResourceId,
                        "slotid_" + i,
                        "objtype_CSlotInfo",
                      ];
                    ...
                    var cObjkey = util.format(
                      "ConcurrencyInfo:%d:%d:%s:%s",
                      preProcessResData.Company,
                      preProcessResData.Tenant,
                      preProcessResData.ResourceId,
                      obj.HandlingType
                    ...
                    if (tempRefInfoObj) {
                      ...
                      tempRefInfoObj.ResourceName =
                        preProcessResData.ResourceName;
                    }
                    ...
                    var cObjTags = [
                      "company_" + concurrencyObj.Company,
                      "tenant_" + concurrencyObj.Tenant,
                      "handlingType_" + concurrencyObj.HandlingType,
                      "resourceid_" + preProcessResData.ResourceId,
                ...
                sci.on("endconcurrencyInfo", function () {
                  var resourceObj = {
                    ...
                    UserName: basicData.UserName,
                    ResourceAttributeInfo:
                      preProcessResData.ResourceAttributeInfo,
                  ...
                  var tag = [
                    ...
                    "tenant_" + resourceObj.Tenant,
                    "class_" + resourceObj.Class,
                    "type_" + resourceObj.Type,
                    "category_" + resourceObj.Category,
                    "resourceid_" + resourceObj.ResourceId,
                    "objtype_Resource",
                  ];
                  ...
                  for (var i in resourceObj.ResourceAttributeInfo) {
                    tempAttributeList.push(
                      resourceObj.ResourceAttributeInfo[i].Attribute
                    );
                  }
                  ...
                  redisHandler.AddObj_V_T(logKey, key, jsonObj, tag, function (
                    err,
                    reply,
                    vid
                  ) {
                    var resource_issMapKey = util.format(
                      "ResourceIssMap:%d:%d:%s",
                      resourceObj.Company,
                      resourceObj.Tenant,
                      resourceObj.UserName
                    );
                    redisHandler.SetObj_NX(
                      logKey,
                      resource_issMapKey,
                      key,
                      function () {}
                    ...
                    resourceStateMapper.SetResourceState(
                      ...
                      function (err, result) {
                        if (
                          resourceObj.ConcurrencyInfo &&
                          resourceObj.ConcurrencyInfo.length === 0
                        ) {
                          resourceStateMapper.SetResourceState(
                            logKey,
                            resourceObj.Company,
                    ...
                    logger.info(
                      "%s Finished AddResource. Result: %s",
                      logKey,
                      reply
                    );
...
var AddResource = function (logKey, basicData, callback) {
  ...
  redisHandler.CheckObjExists(logKey, resourceKey, function (err, isExists) {
    if (isExists == "1") {
      RemoveResource(
        logKey,
        basicData.Company.toString(),
        basicData.Tenant.toString(),
        basicData.ResourceId,
        function (err, result) {
        ...
        sci.on("concurrencyInfo", function (obj) {
          ...
          if (validateHandlingType) {
            ...
            redisHandler.GetObj(logKey, cObjkey, function (cErr, isExists) {
              ...
              var cObjTags = [
                "company_" + basicData.Company,
                "tenant_" + basicData.Tenant,
                "handlingType_" + concurrencyObj.HandlingType,
                "resourceid_" + preProcessResData.ResourceId,
                "objtype_ConcurrencyInfo",
              ];
              ...
              var jsonConObj = JSON.stringify(concurrencyObj);
              if (
                loginTasks.indexOf(concurrencyObj.HandlingType) == -1 ||
                !isExists ||
                editType == "addResource"
              ) {
                ...
                redisHandler.GetObj(logKey, tagMetaKey, function (err, ceTags) {
                  if (ceTags) {
                    var newCompany = util.format(
                      "company_%s",
                      basicData.Company
                    );
                    commonMethods.AppendNewCompanyTagStr(
                      ceTags,
                      newCompany,
                      function (newTags) {
                        redisHandler.SetTags(
                          logKey,
                          newTags,
                          cObjkey,
                          function (err, reply) {
              ...
              for (var i = 0; i < obj.NoOfSlots; i++) {
                ...
                if (!isExists || editType == "addResource") {
                  ...
                  redisHandler.GetObj(logKey, slotTagMetaKey, function (
                    err,
                    seTags
                  ) {
                    if (seTags) {
        ...
        sci.on("endconcurrencyInfo", function () {
          ...
          redisHandler.GetObj(logKey, tagMetaKey, function (err, reTags) {
            if (reTags) {
              ...
              commonMethods.AppendNewCompanyTagArray(
                reTags,
                newCompany,
                function (newTags) {
                  tag = newTags;
                  ...
                  redisHandler.SetObj_V_T(
                    ...
                    function (err, reply, vid) {
                      logger.info(
                        "%s Finished SetResource. Result: %s",
                        logKey,
                        reply
                      );
                      ...
                      ardsMonitoringService.SendResourceStatus(
                        accessToken,
                        resourceObj.ResourceId,
                        undefined
                      );
          ...
          resourceStateMapper.SetResourceState(
            ...
            function (err, result) {
              redisHandler.RemoveObj_V_T(logKey, key, tag, function (
                ...
                if (err) {
                  logger.info(
                    "%s Finished RemoveResource. Result: %s",
                    logKey,
                    "false"
                  );
                  callback(err, "false");
                } else {
                  var accessToken = util.format(
                    "%d:%d",
                    resourceObj.Tenant,
                    resourceObj.Company
                  );
                  var pubAdditionalParams = util.format(
                    "resourceName=%s&statusType=%s",
...
var RemoveShareResource = function (
  logKey,
  company,
  tenant,
  resourceId,
    ...
    if (err) {
      ...
      if (strObj && Array.isArray(strObj) && strObj.length > 0) {
        ...
        PreProcessResourceData(
          ...
          preResourceData,
          htArray,
          function (err, msg, preProcessResData, attributeToRemove) {
            if (err) {
              callback(err, msg, null);
            } else {
              var sci = SetConcurrencyInfo(preProcessResData.ConcurrencyInfo);
              ...
              sci.on("concurrencyInfo", function (obj) {
                ...
                if (obj.HandlingType == handlingType) {
                  ...
                  redisHandler.CheckObjExists(logKey, cObjkey, function (
                    cErr,
                    isExists
                  ) {
                    if (cErr) {
                    ...
                    if (isExists == 1) {
                      var tagMetaKey = util.format("tagMeta:%s", cObjkey);
                      redisHandler.GetObj(logKey, tagMetaKey, function (
                        err,
                        ceTags
                      ) {
                        if (ceTags) {
                          var newCompany = util.format("company_%s", company);
                          commonMethods.RemoveTagFromTagStr(
                            ceTags,
                            newCompany,
                            function (newTags) {
                              if (newTags.includes("company_")) {
                                ...
                                commonMethods.ConvertTagStrToArray(
                                  newTags,
                                  function (slotInfoTags) {
                                    redisHandler.RemoveObj_V_T(
                                      logKey,
                                      cObjkey,
                                      slotInfoTags,
                    ...
                    function setSlotDetail(slotInfokey, setSlotDetailCallback) {
                      ...
                      if (isExists == 1) {
                        var slotTagMetaKey = util.format(
                          "tagMeta:%s",
                          slotInfokey
                        );
                        redisHandler.GetObj(logKey, slotTagMetaKey, function (
                    ...
                    for (var i = 0; i < obj.NoOfSlots; i++) {
                      ...
                      setSlotDetails.push(
                        setSlotDetail.bind(this, slotInfokey)
                      );
                    }
                  ...
                  if (reTags) {
                    ...
                    if (attributeToRemove && attributeToRemove.length > 0) {
                      for (var i in attributeToRemove) {
                        if (
                          handlingTypesToRemove.indexOf(
                            attributeToRemove[i].HandlingType
                          ) === -1
                        ) {
                          handlingTypesToRemove.push(
                            attributeToRemove[i].HandlingType
                          );
                        }
                        tagsToRemove.push(
                          "attribute_" + attributeToRemove[i].Attribute
                        );
                        resourceObj.ResourceAttributeInfo = commonMethods.RemoveItemFromObjectArray(
                          resourceObj.ResourceAttributeInfo,
                          "Attribute",
                          attributeToRemove[i].Attribute
                        );
                      }
                    } else {
                    ...
                    for (var k in handlingTypesToRemove) {
                      for (var j in cInfoCopy) {
                        var cIndex = cInfoCopy[j].indexOf(
                          handlingTypesToRemove[k]
                        );
                        if (cIndex > -1) {
                          var removeIndex = resourceObj.ConcurrencyInfo.indexOf(
                            cInfoCopy[j]
                          );
                          if (removeIndex > -1) {
                    ...
                    commonMethods.RemoveTagsFromTagArray(
                      reTags,
                      tagsToRemove,
                      function (newTags) {
                        var jsonObj = JSON.stringify(resourceObj);
  ...
  redisHandler.GetObj_V(logKey, key, function (err, jobj, cVid) {
    if (err) {
      ...
      if (jobj) {
        ...
        callback(
          new Error("No registerd resource found"),
          undefined,
          undefined
        );
      }
    ...
    if (err) {
      ...
      if (result) {
        ...
        if (
          ...
          redisHandler.MGetObj(logKey, resourceObj.ConcurrencyInfo, function (
            err,
            cObjs
          ) {
...
var GetMyChats = function (
  ...
  var key = util.format("socialChats:%s:%s:%s",tenant, company, resourceId);
...
var SearchResourcebyTags = function (logKey, tags, callback) {
  ...
  if (Array.isArray(tags)) {
    ...
    redisHandler.SearchObj_V_T(logKey, tags, function (err, result) {
      logger.info(
        "%s Finished SearchResourcebyTags. Result: %s",
        logKey,
        result
      );
      callback(err, result);
...
var UpdateLastConnectedTime = function (
  logKey,
  company,
  tenant,
  handlingType,
  ...
  logger.info(
    "%s ************************* Start UpdateLastConnectedTime *************************",
    logKey
  );
  ...
  redisHandler.GetObj_V(logKey, cObjkey, function (err, obj, vid) {
    if (err) {
      ...
      redisHandler.GetObj(logKey, tagMetaKey, function (err, ceTags) {
        ...
        if (ceTags) {
          commonMethods.GetSortedCompanyTagArray(ceTags, function (
            companyTags
          ) {
            var cObj = JSON.parse(obj);
            if (event == "reserved") {
              if (maxRejectCount) cObj.MaxRejectCount = maxRejectCount;
            ...
            var tags = [
              "tenant_" + cObj.Tenant,
              "handlingType_" + cObj.HandlingType,
              "resourceid_" + cObj.ResourceId,
              "objtype_ConcurrencyInfo",
            ];
            ...
            redisHandler.SetObj_V_T(
              ...
              function () {
                logger.info(
                  "%s Finished UpdateLastConnectedTime. Result: %s",
                  logKey,
                  result
                );
...
var UpdateRejectCount = function (
  logKey,
  company,
  tenant,
  handlingType,
          ...
          commonMethods.GetSortedCompanyTagArray(ceTags, function (
            ...
            if (cObj.RejectCount >= cObj.MaxRejectCount) {
              ...
              var notificationMsg = {
                ...
                To: cObj.UserName,
                Message:
                  "Reject Count Exceeded!, Account Suspended for Task: " +
                  cObj.HandlingType,
              };
              notificationService.SendNotificationInitiate(
                logKey,
                "message",
                "",
                notificationMsg,
            ...
            redisHandler.SetObj_V_T(
              ...
              function () {
                logger.info(
                  "%s Finished UpdateRejectCount. Result: %s",
                  logKey,
                  result
                );
                callback(err, result, vid);
            ...
            resourceService.AddResourceTaskRejectInfo(
              internalToken,
              cObj.BusinessUnit,
              cObj.ResourceId,
              cObj.HandlingType,
...
var UpdateSlotStateAvailable = function (
  logKey,
  company,
  tenant,
  handlingType,
  ...
  logger.info(
    "%s ************************* Start UpdateSlotStateAvailable *************************",
    logKey
  );
              ...
              if (ceTags) {
                commonMethods.GetSortedCompanyTagArray(ceTags, function (
                  ...
                  if (tempObj.FreezeAfterWorkTime) {
                    try {
                      scheduleWorkerHandler.endFreeze(
                        company,
                        tenant,
                        resourceid,
                        logKey
                      ...
                      logger.error(
                        "scheduleWorkerHandler.endFreeze Error :: " + ex
                      );
                    }
                  }
                  ...
                  tempObj.OtherInfo = "";
                  var tags = [
                    "tenant_" + tempObj.Tenant,
                    "handlingType_" + tempObj.HandlingType,
                    "state_" + tempObj.State,
                    "resourceid_" + tempObj.ResourceId,
                    "slotid_" + tempObj.SlotId,
                    "objtype_CSlotInfo",
                  ];
                  ...
                  redisHandler.SetObj_V_T(
                    ...
                    function (err, reply, vid) {
                      logger.info(
                        "%s Finished UpdateSlotStateAvailable. Result: %s",
                        logKey,
                        reply
                      );
                      if (err != null) {
                        ...
                        var duration = moment(tempObj.StateChangeTime).diff(
                          moment(tempObjCopy.StateChangeTime),
                          "seconds"
                        );
                        var internalAccessToken = util.format(
                          "%s:%s",
                          tenant,
                          company
                        );
                        ...
                        resourceService.AddResourceStatusChangeInfo(
                          internalAccessToken,
                          ...
                          function (err, result, obj) {
                            if (err) {
                              logger.error(
                                "AddResourceStatusChangeInfo Failed.",
                                err
                              );
                            } else {
                              logger.info(
                                "AddResourceStatusChangeInfo Success.",
                                obj
                              );
                              resourceService.AddResourceStatusDurationInfo(
                                internalAccessToken,
                                tempObj.BusinessUnit,
                                tempObj.ResourceId,
                                "SloatStatus",
                                tempObjCopy.State,
                                "",
                                tempObjCopy.OtherInfo,
                                tempObjCopy.HandlingRequest,
                                duration,
                                function () {
                                  if (err) {
                                    logger.error(
                                      "AddResourceStatusDurationInfo Failed.",
                                      err
                                    );
                                  } else {
                                    logger.info(
                                      "AddResourceStatusDurationInfo Success.",
                                      obj
                                    );
                                  }
                                }
...
var UpdateSlotStateAfterWork = function (
  logKey,
  company,
  tenant,
  handlingType,
  ...
  logger.info(
    "%s ************************* Start UpdateSlotStateAfterWork *************************",
    logKey
  );
            ...
            commonMethods.GetSortedCompanyTagArray(ceTags, function (
              ...
              if (
                ...
                var tags = [
                  ...
                  "resourceid_" + tempObj.ResourceId,
                  "slotid_" + tempObj.SlotId,
                  "handlingrequest_" + tempObj.HandlingRequest,
                  "objtype_CSlotInfo",
                ];
                var slotInfoTags = companyTags.concat(tags);
                var jsonObj = JSON.stringify(tempObj);
                redisHandler.SetObj_V_T(
                  ...
                  function (err, reply, vid) {
                    logger.info(
                      "%s Finished UpdateSlotStateAfterWork. Result: %s",
                      logKey,
                      reply
                    );
                    if (err != null) {
...
var UpdateSlotStateReserved = function (
  logKey,
  company,
  tenant,
  businessUnit,
  ...
  logger.info(
    "%s ************************* Start UpdateSlotStateReserved *************************",
    logKey
  );
        ...
        redisHandler.GetObj(logKey, tagMetaKey, function (err, ceTags) {
          ...
          if (ceTags) {
            commonMethods.GetSortedCompanyTagArray(ceTags, function (
              ...
              if (maxReservedTime) tempObj.MaxReservedTime = maxReservedTime;
              if (maxAfterWorkTime) tempObj.MaxAfterWorkTime = maxAfterWorkTime;
              if (maxFreezeTime) tempObj.MaxFreezeTime = maxFreezeTime;
              var tags = [
                "tenant_" + tempObj.Tenant,
                "handlingType_" + tempObj.HandlingType,
                "state_" + tempObj.State,
              ...
              redisHandler.SetObj_V_T(
                ...
                function (err, reply, vid) {
                  logger.info(
                    "%s Finished UpdateSlotStateReserved. Result: %s",
                    logKey,
                    reply
                  );
                  if (err != null) {
...
var UpdateSlotStateConnected = function (
  logKey,
  company,
  tenant,
  handlingType,
  ...
  logger.info(
    "%s ************************* Start UpdateSlotStateConnected *************************",
    logKey
  );
          ...
          if (ceTags) {
            commonMethods.GetSortedCompanyTagArray(ceTags, function (
              ...
              redisHandler.SetObj_V_T(
                ...
                function (err, reply, vid) {
                  logger.info(
                    "%s Finished UpdateSlotStateConnected. Result: %s",
                    logKey,
                    reply
                  );
                  if (err != null) {
...
var UpdateSlotStateCompleted = function (
  ...
  UpdateSlotStateAfterWork(
    ...
    function (err, reply) {
      ...
      if (err) {
        logger.info(
          "******************Update Slot State 'AfreWork' Failed, Forcefully Put in Resource to Available State"
        );
        ...
        logger.info(
          "******************Update Slot State 'AfreWork', Slot already in Afterwork State"
        );
      } else {
        var slotInfokey = util.format(
          "CSlotInfo:%s:%s:%s:%s:%s",
...
var SetSlotStateFreeze = function (
  logKey,
  company,
  tenant,
  handlingType,
    ...
    redisHandler.GetObj_V(logKey, slotInfokey, function (err, obj, vid) {
      if (err) {
        ...
        if (tempObj.State === "AfterWork") {
            ...
            if (ceTags) {
              commonMethods.GetSortedCompanyTagArray(ceTags, function (
                ...
                redisHandler.SetObj_V_T(
                  ...
                  function (err, reply, vid) {
                    logger.info(
                      "%s Finished SetSlotStateFreeze. Result: %s",
                      logKey,
                      reply
                    );
                    if (err != null) {
                    ...
                    try {
                      scheduleWorkerHandler.startFreeze(
                        company,
                        tenant,
                        resourceid,
                        resourceid,
                        tempObj.MaxFreezeTime,
                        tempObj.HandlingRequest,
                        logKey
                      );
                    } catch (ex) {
          ...
          callback(
            new Error("Cannot Freeze, Resource not in AfterWork State"),
            null
          );
        }
      }
...
var UpdateSlotStateBySessionId = function (
  logKey,
  company,
  tenant,
  handlingType,
  ...
  if (
    (direction === "outbound" || direction === "inbound_internal") &&
    (state.toLowerCase() === "reserved" || state.toLowerCase() === "connected")
  ) {
    slotInfoTags = [
      "company_" + company,
      "tenant_" + tenant,
      "handlingType_" + handlingType,
      "resourceid_" + resourceid,
    ];
        ...
        if (cslots != null && cslots.length > 0) {
          ...
          if (selectedSlot) {
            if (state.toLowerCase() === "connected") {
              UpdateSlotStateConnected(
                logKey,
                selectedSlot.Company,
                selectedSlot.Tenant,
                selectedSlot.HandlingType,
              ...
              state.toLowerCase() === "reserved" &&
              selectedSlot.State !== "Connected"
            ) {
              UpdateSlotStateReserved(
                logKey,
                selectedSlot.Company,
                selectedSlot.Tenant,
                selectedSlot.BusinessUnit,
                selectedSlot.HandlingType,
                selectedSlot.ResourceId,
                selectedSlot.SlotId,
                sessionid,
              ...
              callback(
                new Error("Invalid outbound status change request"),
                undefined
              );
            }
          } else {
    ...
    SearchCSlotByTags(logKey, slotInfoTags, function (err, cslots) {
      if (err) {
        ...
        if (state == "Reject") {
          ...
          var companyInt = parseInt(company);
          var businessUnit =
            cslots && cslots.length > 0 && cslots[0] && cslots[0].Obj
              ? cslots[0].Obj.BusinessUnit
              : "default";
          dashboardEventHandler.PublishEvent(
            logKey,
            tenantInt,
        ...
        if (cslots && cslots.length > 0) {
          ...
          callback(
            new Error(
              "No Reserved Resource CSlot found for sessionId: " + sessionid
            ),
            undefined
          );
        }
...
var SearchCSlotByTags = function (logKey, tags, callback) {
  ...
  if (Array.isArray(tags)) {
    ...
    e.message = "tags must be a string array";
    logger.info(
      "%s Finished SearchCSlotByTags. Result: %s",
      logKey,
      "tags must be a string array"
    );
    callback(e, null);
...
var SearchConcurrencyInfoByTags = function (logKey, tags, callback) {
  logger.info(
    "%s ************************* Start SearchConcurrencyInfoByTags *************************",
    logKey
  );
  ...
  if (Array.isArray(tags)) {
    ...
    redisHandler.SearchObj_V_T(tags, function (err, result) {
      logger.info(
        "%s Finished SearchConcurrencyInfoByTags. Result: %s",
        logKey,
        result
      );
      callback(err, result);
...
var DoResourceSelection = function (
  company,
  tenant,
  resourceCount,
  sessionId,
  ...
  if (
    ...
    validator.isIP(config.Services.routingServiceHost)
  ) {
    rUrl = util.format(
      "http://%s:%s",
      config.Services.routingServiceHost,
      config.Services.routingServicePort
    );
  }
  var params = util.format(
    "/resourceselection/getresource/%d/%d/%d/%s/%s/%s/%s/%s/%s",
    company,
    tenant,
    resourceCount,
    sessionId,
