  ...
  try {
    if (
      ...
      if (
        ...
        validator.isIP(config.Services.cronurl)
      )
        cronURL = format(
...
function stopCronJob(company, tenant, id, cb) {
  try {
    if (
      ...
      var cronURL = format(
        "http://{0}/DVP/API/{1}/Cron/Reference/{2}/Action/destroy",
        config.Services.cronurl,
        config.Services.cronversion,
        id
      );
      if (
        ...
        cronURL = format(
          "http://{0}:{1}/DVP/API/{2}/Cron/Reference/{3}/Action/destroy",
          config.Services.cronurl,
          config.Services.cronport,
          config.Services.cronversion,
          id
...
module.exports.startBreak = function (
  company,
  tenant,
  userName,
  resourceId,
  ...
  try {
    ...
    if (config.Services.dynamicPort || validator.isIP(config.Host.LBIP))
      mainServer = format(
        "http://{0}:{1}/DVP/API/{2}/ARDS/Notification/{3}",
        config.Host.LBIP,
        config.Host.LBPort,
        config.Host.Version,
        userName
...
module.exports.startFreeze = function (
  company,
  tenant,
  userName,
  resourceId,
